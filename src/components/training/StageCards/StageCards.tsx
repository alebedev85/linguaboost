...
export default function StageCards({ word, onAnswer }: StageCardsProps) {
  return (
    <div className={styles.innerWrapper}>
      <div className={styles.gridContainer}>
        ...
        <div className={styles.infoZone}>
          <div className={styles.wordHeader}>
            <h3 className={styles.englishWord}>{word.english}</h3>
          ...
          <div className={styles.metaGroup}>
            ...
            <p className={styles.russianWord}>{word.russian}</p>
          </div>

          {word.context && (
            <div className={styles.metaGroup}>
              <span className={styles.metaLabel}>Контекст</span>
              <p className={styles.contextBubble}>&quot;{word.context}&quot;</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className={styles.actionsGrid}>
        <button onClick={() => onAnswer(false)} className={styles.btnDanger}>
          Не помню 😢
        </button>
        <button onClick={() => onAnswer(true)} className={styles.btnSuccess}>
          Запомнил 👍
        </button>
      </div>
    </div>
